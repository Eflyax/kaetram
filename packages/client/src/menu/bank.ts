...
export default class Bank extends Menu {
	...
	private load(): void {
		...
		for (let i = 0; i < Modules.Constants.BANK_SIZE; i++) {
			let slot = this.draggableSlot(i, Modules.ContainerType.Bank, Modules.ContainerType.Inventory);

			this.bankList.append(slot);
		}

		for (let i = 0; i < Modules.Constants.INVENTORY_SIZE; i++) {
			let slot = this.draggableSlot(i, Modules.ContainerType.Inventory, Modules.ContainerType.Bank);
