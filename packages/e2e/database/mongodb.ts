...
export default class MongoDB {
	...
	public upsert(
		...
		collection.updateOne(filter, { $set: body }, { upsert: true }, (error, result) => {
			if (error)
				log.error(
					`An error occurred while saving ${collection.collectionName} for ${JSON.stringify(
						filter
					)}:`,
					error
				);
			...
			if (!result)
				log.error(`Unable to save ${collection.collectionName} for ${JSON.stringify(filter)}.`);
			callback(error);
		});
	}
